  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `
          ...
          <div style="padding: 30px; background-color: #ffffff; color: #2c2c2c;">
            <h2 style="color: #2c2c2c; margin: 0; text-align: center; margin-bottom: 30px;">RECUPERAR SENHA</h2>
            <p style="font-size: 16px;">Olá ${user.Nome || ""},</p>
            <p style="font-size: 16px;">Estamos quase lá! Basta clicar no botão abaixo para recuperar sua senha:</p>
            <div style="text-align: center; margin: 30px 0;">
              <a href="${link}" style="background-color: #16aa51; color: #fff; padding: 10px 20px; text-decoration: none; border-radius: 12px; font-size: 16px;">RECUPERAR SENHA</a>
            </div>
            <p style="font-size: 14px; color:rgb(221, 45, 45); margin-bottom: 20px;">Se você não solicitou essa alteração, ignore esse e-mail.</p>
            <p style="font-size: 14px; color: #777;">Se o botão acima não funcionar, copie e cole o seguinte link no seu navegador:</p>
            <p style="font-size: 14px; word-break: break-all;">${link}</p>
          </div>
          <div style="background-color: #f1f5f9; padding: 15px; text-align: center; font-size: 12px; color: #666; margin-bottom: 10px;">
...
export const recuperarSenha: RequestHandler<void> = async (req, res, next) => {
  const body = req.body;
...
export async function verificarPrimeiroAcesso(req: Request, res: Response) {
  const userId = req.user?.User_ID;

  if (!userId) {
    res
      .status(400)
      .json({ success: false, error: "Usuário inválido ou não autenticado" });
    return;
  }
  ...
  try {
    const [rows]: any = await connection
      .promise()
      .query("SELECT Primeiro_Acesso, Email FROM Users WHERE User_ID = ?", [userId]);
    ...
    if (rows[0].Primeiro_Acesso === 0) {
      res
        .status(400)
        .json({ success: false, error: "O primeiro acesso já foi verificado", primeiroAcesso: false });
      return;
    }
    ...
    registrarLog(`O usuário ${rows[0].Email} acessou a conta pela primeira vez`, userId);

    res.json({ success: true, message: "Primeiro acesso verificado com sucesso", primeiroAcesso: true });
  } catch (error) {
    console.error("Erro ao verificar o primeiro acesso:", error);
    res
      .status(500)
      .json({ success: false, error: "Erro ao verificar o primeiro acesso" });
  }
}