...
const GerenciarManifestacao = () => {
  ...
  const fetchRespostas = async () => {
    ...
    const response = await getRespostasManifestacao(Number(id));
  ...
  useEffect(() => {
    ...
    const fetchManifestacao = async () => {
      try {
        const response = await getManifestacao(Number(id));
                  ...
                  <CardHeader className="px-4 flex flex-row items-center justify-between">
                    <div className="flex flex-row items-center">
                      <Avatar
                        onClick={() =>
                          navigate(
                            `/admin/gerenciar/${resposta.User_ID}/perfil`
                          )
                        }
                      >
                        <AvatarImage
                          ...
                          alt="Ícone do Usuário"
                          className="w-8 h-8 aspect-square cursor-pointer rounded-full"
                        />
                      </Avatar>
                      <div className="ml-2">
